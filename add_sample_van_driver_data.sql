-- Add Sample Van and Driver Data for Testing
-- Run this after setting up authentication

BEGIN;

-- First, let's add some sample van profiles
...
('VAN001', 'Ford', 'Transit', 2022, 'WF0XXTTGXMWA12345', 'ABC123', 'active', 'Downtown Depot', 45000, '2024-01-15', '2024-07-15', NULL, 'INS-2024-001', '2025-03-15', '2022-01-10', 35000, NOW(), NOW()),
('VAN002', 'Mercedes', 'Sprinter', 2023, 'WD3PE7CD7NP123456', 'DEF456', 'active', 'North Branch', 23000, '2024-02-10', '2024-08-10', NULL, 'INS-2024-002', '2025-04-20', '2023-02-15', 42000, NOW(), NOW()),
('VAN003', 'Ford', 'E-Series', 2021, 'WF0XXTTGXMWA54321', 'GHI789', 'maintenance', 'Service Center', 67000, '2024-03-01', '2024-09-01', NULL, 'INS-2024-003', '2025-01-30', '2021-03-20', 32000, NOW(), NOW()),
('VAN004', 'Chevrolet', 'Express', 2022, '1GCWGBFG8N1234567', 'JKL012', 'active', 'East Side', 38000, '2024-01-20', '2024-07-20', NULL, 'INS-2024-004', '2025-05-10', '2022-04-05', 34000, NOW(), NOW()),
('VAN005', 'Ram', 'ProMaster', 2023, '3C6TRVBG5NE123456', 'MNO345', 'out_of_service', 'Repair Shop', 15000, '2024-02-25', '2024-08-25', NULL, 'INS-2024-005', '2025-06-15', '2023-05-12', 38000, NOW(), NOW()),
('VAN006', 'Ford', 'Transit', 2022, 'WF0XXTTGXMWA98765', 'PQR678', 'active', 'West Branch', 41000, '2024-01-30', '2024-07-30', NULL, 'INS-2024-006', '2025-02-28', '2022-06-18', 36000, NOW(), NOW()),
('VAN007', 'Mercedes', 'Sprinter', 2023, 'WD3PE7CD7NP987654', 'STU901', 'active', 'Central Hub', 19000, '2024-03-05', '2024-09-05', NULL, 'INS-2024-007', '2025-07-22', '2023-07-03', 43000, NOW(), NOW()),
('VAN008', 'Ford', 'E-Series', 2021, 'WF0XXTTGXMWA13579', 'VWX234', 'maintenance', 'Maintenance Bay 2', 72000, '2024-02-15', '2024-08-15', NULL, 'INS-2024-008', '2025-03-12', '2021-08-14', 31000, NOW(), NOW());

-- Add some sample driver profiles
...
-- Add some sample van assignments
UPDATE public.van_profiles SET driver_id = (SELECT id FROM driver_profiles WHERE driver_name = 'John Smith' LIMIT 1) WHERE van_number = 'VAN001';
UPDATE public.van_profiles SET driver_id = (SELECT id FROM driver_profiles WHERE driver_name = 'Maria Rodriguez' LIMIT 1) WHERE van_number = 'VAN002';
UPDATE public.van_profiles SET driver_id = (SELECT id FROM driver_profiles WHERE driver_name = 'David Johnson' LIMIT 1) WHERE van_number = 'VAN004';
UPDATE public.van_profiles SET driver_id = (SELECT id FROM driver_profiles WHERE driver_name = 'Lisa Chen' LIMIT 1) WHERE van_number = 'VAN006';
UPDATE public.van_profiles SET driver_id = (SELECT id FROM driver_profiles WHERE driver_name = 'Robert Wilson' LIMIT 1) WHERE van_number = 'VAN007';

-- Add some sample van images with damage reports
...
LEFT JOIN public.driver_profiles d ON v.driver_id = d.id
LEFT JOIN public.van_images vi ON v.van_number = vi.van_number
GROUP BY v.van_number, v.make, v.model, v.status, d.driver_name
