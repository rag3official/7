...
def assess_damage_with_claude(image_url: str) -> dict:
    ...
    try:
        logger.info(f"Starting damage assessment for image: {image_url}")
        ...
        data = {
            ...
            "messages": [{
                ...
                "content": [
                    {
                        "type": "text",
                        "text": """Please analyze this specific van image for damage. Be very detailed and specific about what you see.
                        Rate any damage on a scale of 0-3:
                        0 - No visible damage
                        1 - Minor issues (dirt, dust, small scratches)
                        2 - Moderate damage (multiple scratches, small dents)
                        3 - Severe damage (large dents, major damage)
                        
                        Focus only on the image provided and describe the exact damage you see.
                        Include specific locations and details of the damage.
                        
                        Provide your assessment in a JSON format with:
                        - damage_level (0-3)
                        - description (detailed explanation of specific damage seen)
                        - location (specific area of damage on van)
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Received response from Claude for image {image_hash}: {content}")
            ...
            if json_match:
                ...
                required_fields = ['damage_level', 'description', 'location', 'confidence']
                if not all(field in assessment for field in required_fields):
                    raise Exception("Missing required fields in assessment")
                
                # Add image tracking information
                assessment['image_hash'] = image_hash
                assessment['image_url'] = image_url
                logger.info(f"Final assessment for image {image_hash}: {assessment}")
                return assessment
            else:
            ...
            return {
                "damage_level": 0,
                "description": "Error assessing damage - API call failed",
                "location": "Unknown",
                "confidence": "low",
        ...
        return {
            "damage_level": 0,
            "description": f"Error assessing damage: {str(e)}",
            "location": "Unknown",
            "confidence": "low",
...
def save_image_assessment(van_id: str, image_url: str, image_hash: str, assessment: dict) -> dict:
    """Save image assessment to van_images table and return the created record."""
    try:
        logger.info(f"Saving image assessment for van {van_id}")
        
        # Check if we already have an image with this hash for this van
        ...
        if existing_images.data:
            logger.info(f"Found existing image record with hash {image_hash}")
            ...
            result = supabase.table('van_images').update(image_data).eq('image_hash', image_hash).eq('van_id', van_id).execute()
        else:
        ...
        if not result.data:
            raise Exception("Failed to save image assessment")
            
        saved_image = result.data[0]
        logger.info(f"Saved image assessment with ID: {saved_image['id']}")
        return saved_image
        
    except Exception as e:
        logger.error(f"Error saving image assessment: {e}")
        raise
...
def update_van_damage(van_id: str, damage_assessment: dict, image_url: str) -> None:
    ...
    try:
        ...
        # Save image assessment first
        image_record = save_image_assessment(
            van_id=van_id,
            image_url=image_url,
            image_hash=damage_assessment.get('image_hash'),
            assessment=damage_assessment
        )
        
        # Get all images for this van to determine highest damage level
        van_images = supabase.table('van_images').select('*').eq('van_id', van_id).execute()
        if van_images.data:
            current_rating = max(float(img.get('damage_level', 0)) for img in van_images.data)
        else:
            current_rating = 0
        ...
        # Set status to Maintenance if any image shows severe damage
        if current_rating == 3:
            update_data['status'] = 'Maintenance'
            
        # Update van in database
        supabase.table('vans').update(update_data).eq('id', van_id).execute()
        logger.info(f"Updated van {van_id} with new image assessment {image_record['id']}")
