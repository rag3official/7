...
logger.info('🚀 Starting RAW HTTP UPLOAD Slack Bot...')
logger.info('📁 Focus: Use raw HTTP requests to bypass Supabase client constraints')
...
def upload_to_supabase_raw(file_path, image_data, content_type):
    """Upload file to Supabase using raw HTTP requests"""
    try:
        logger.info(f'🌐 RAW HTTP upload to: {file_path}')
        ...
        if response.status_code in [200, 201]:
            logger.info(f'✅ RAW HTTP upload successful!')
            return True
        else:
            logger.error(f'❌ RAW HTTP upload failed: {response.status_code}')
            logger.error(f'❌ Response text: {response.text}')
            return False
            
    except Exception as e:
        logger.error(f'❌ RAW HTTP upload exception: {e}')
        return False
...
def process_file_upload(client, file_info, channel_id=None, event_type="unknown"):
    ...
    try:
        ...
        upload_success = upload_to_supabase_raw(file_path, image_data, detected_content_type)
        ...
        if upload_success:
            ...
            try:
                client.chat_postMessage(
                    channel=channel_id,
                    text=f"❌ Failed to upload image for van #{van_number}. Please try again."
                )
