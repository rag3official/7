#!/usr/bin/env python3
"""
Enhanced Slack Bot with Claude AI Integration - SCHEMA FIXED VERSION
Automatically processes van images uploaded to Slack and analyzes them with Claude AI.
"""
...
def get_or_create_van(van_number: str) -> Tuple[str, Dict[str, Any]]:
    """Get existing van or create new one if it doesn't exist - SCHEMA FIXED VERSION."""
    try:
        ...
        # Create new van with CORRECT schema (only columns that exist)
        new_van = {
            'van_number': van_number,
            'type': 'Transit',
            'status': 'Active',
            'driver': 'Unassigned',
            'maintenance_notes': f'Van #{van_number} created via Slack bot',
            'created_at': datetime.now().isoformat(),
            'updated_at': datetime.now().isoformat()
        }
        
        logger.info(f"🆕 Creating new van #{van_number} with schema: {list(new_van.keys())}")
...
def assess_damage_with_claude(image_data: bytes, filename: str) -> Dict[str, Any]:
    """Analyze image with Claude AI for damage assessment - WITH JPG CONVERSION."""
    try:
        logger.info(f"🔍 Analyzing image with Claude AI: {filename}")
        
        # Convert all images to JPG format for Claude AI
        logger.info(f"📸 Original image signature: {image_data[:20]}")
        logger.info(f"📏 Original image size: {len(image_data)} bytes")
        
        # Use PIL to convert image to JPEG
        image = Image.open(io.BytesIO(image_data))
        logger.info(f"🖼️ PIL opened image: format={image.format}, size={image.size}, mode={image.mode}")
        ...
        logger.info(f"📸 Final image signature: {jpeg_data[:20]}")
        
        # Always use JPEG media type after conversion
        media_type = "image/jpeg"
        logger.info(f"📋 Final media type for Claude: {media_type}")
        ...
        logger.info(f"✅ Damage assessment completed: {damage_assessment}")
...
def store_van_image(van_id: str, van_number: str, image_data: bytes, filename: str, damage_assessment: Dict[str, Any]) -> str:
    """Store van image in database with damage assessment - SCHEMA FIXED VERSION."""
    try:
        ...
        # Prepare van_images record with CORRECT schema
        van_image_record = {
            'van_id': van_id,
            'image_url': image_data_url,  # Store as data URL
        ...
        logger.info(f"�� Storing van image with schema: {list(van_image_record.keys())}")
...
def process_image_file(file_info: Dict[str, Any], van_number: str, say, client) -> None:
    """Process an image file for damage assessment - SCHEMA FIXED VERSION."""
    try:
...
if __name__ == "__main__":
    logger.info("🚀 Starting CLAUDE AI ENHANCED Slack Bot (SCHEMA FIXED)...")
