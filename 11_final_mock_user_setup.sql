-- Final Mock User Setup for Testing
-- This script uses the correct unique constraint (email)
...
BEGIN
    ...
    IF admin_auth_id IS NOT NULL THEN
        -- Insert into public.users (using email for conflict resolution since it has unique constraint)
        ...
        -- Get the public.users id for subscription
        SELECT id INTO admin_user_id FROM public.users WHERE email = 'testadmin@vantracker.com';
        ...
        RAISE NOTICE 'Created admin user profile for: % (user_id: %)', admin_auth_id, admin_user_id;
    ELSE
...
BEGIN
    ...
    IF crew_auth_id IS NOT NULL THEN
        ...
        SELECT id INTO crew_user_id FROM public.users WHERE email = 'testcrew@vantracker.com';
        ...
        RAISE NOTICE 'Created crew user profile for: % (user_id: %)', crew_auth_id, crew_user_id;
    ELSE
...
BEGIN
    ...
    IF viewer_auth_id IS NOT NULL THEN
        ...
        SELECT id INTO viewer_user_id FROM public.users WHERE email = 'testviewer@vantracker.com';
        ...
        RAISE NOTICE 'Created viewer user profile for: % (user_id: %)', viewer_auth_id, viewer_user_id;
    ELSE
