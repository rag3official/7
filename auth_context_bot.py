...
logger.info('🚀 Starting AUTH CONTEXT Slack Bot...')
logger.info('📁 Focus: Use authenticated user context to bypass rate limiting')
...
def upload_to_supabase_simple_fallback(file_data, file_path, content_type):
    """Simple fallback upload without any rate limiting triggers"""
    try:
        logger.info(f'🔄 SIMPLE FALLBACK upload to: {file_path}')
        logger.info(f'📤 Content-Type: {content_type}')
        logger.info(f'📤 Data size: {len(file_data)} bytes')
        
        # Try the most basic upload possible - no upsert, no extra headers
        ...
        logger.info(f'🔗 Simple URL: {url}')
        logger.info(f'📋 Simple Headers: {headers}')
        
        response = requests.post(url, data=file_data, headers=headers)
        
        logger.info(f'📊 Simple Response status: {response.status_code}')
        
        if response.status_code in [200, 201]:
            logger.info('✅ Simple upload successful!')
            return True
        else:
            logger.error(f'❌ Simple upload failed: {response.status_code}')
            logger.error(f'❌ Simple Response: {response.text}')
            
            # If it fails because file exists, try with a different name
            if 'already exists' in response.text.lower():
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S_%f')
                extension = content_type.split('/')[-1] if '/' in content_type else 'jpg'
                unique_filename = f'image_{timestamp}.{extension}'
                unique_path = f'van_{file_path.split("/")[0].split("_")[1]}/{unique_filename}'
                
                logger.info(f'🔄 Retrying with unique filename: {unique_path}')
                
                unique_url = f"{os.environ.get('SUPABASE_URL')}/storage/v1/object/{STORAGE_BUCKET}/{unique_path}"
                response = requests.post(unique_url, data=file_data, headers=headers)
                
                if response.status_code in [200, 201]:
                    logger.info('✅ Unique filename upload successful!')
                    return True
                else:
                    logger.error(f'❌ Unique filename upload failed: {response.status_code}')
                    return False
            
            return False
            
    except Exception as e:
        logger.error(f'❌ Simple fallback exception: {e}')
        return False
...
def process_file_upload(client, file_info, channel_id, event_source="unknown"):
    """Process file upload with simple fallback approach"""
    try:
        ...
        # Upload to Supabase with simple fallback approach
        if upload_to_supabase_simple_fallback(image_data, file_path, content_type):
