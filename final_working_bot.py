...
def get_or_create_van(van_number):
    """Get existing van or create new one with proper headers"""
    try:
        # Check if van exists
        ...
        # Create new van with proper headers
        logger.info(f"🆕 Creating new van #{van_number}")
        
        # Add Prefer header to get response data
        ...
        logger.info(f"📝 Van creation response status: {response.status_code}")
        logger.info(f"📝 Van creation response: {response.text[:200]}")
        ...
        if response.status_code == 201:
            if response.text.strip():  # Check if response has content
                try:
                    ...
                    logger.error(f"❌ Failed to parse van creation response: {e}")
                    # Fallback: try to find the van we just created
                    response = requests.get(
                        f"{SUPABASE_URL}/rest/v1/vans",
                        headers=headers,
                        params={'select': '*', 'van_number': f'eq.{van_number}'}
                    ...
                    if response.status_code == 200:
                        vans = response.json()
                        if vans:
                            logger.info(f"✅ Found newly created van via fallback: {vans[0]['id']}")
                            return vans[0]['id']
            else:
                logger.error("❌ Empty response from van creation")
        else:
...
def process_file_upload(file_id, channel_id, say):
    ...
    try:
        ...
        logger.info("=" * 50)
        logger.info("📁 PROCESSING FILE UPLOAD (FINAL VERSION)")
        logger.info("=" * 50)
        ...
        if not van_id:
            ...
            logger.error(f"❌ Could not create/find van #{van_number}")
            return
...
if __name__ == "__main__":
    logger.info("🚀 Starting FINAL WORKING Slack Bot...")
    logger.info("📁 Focus: Complete functionality with proper error handling")
