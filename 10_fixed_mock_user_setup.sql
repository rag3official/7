-- Fixed Mock User Setup for Testing
-- This script matches the actual users table schema
...
BEGIN
    ...
    IF admin_auth_id IS NOT NULL THEN
        -- Insert into public.users (note: id is auto-generated, auth_user_id references auth.users)
        ...
        DECLARE
            admin_user_id UUID;
        BEGIN
            SELECT id INTO admin_user_id FROM public.users WHERE auth_user_id = admin_auth_id;
        ...
        RAISE NOTICE 'Created admin user profile for auth_user_id: %', admin_auth_id;
    ELSE
...
BEGIN
    ...
    IF crew_auth_id IS NOT NULL THEN
        ...
        DECLARE
            crew_user_id UUID;
        BEGIN
            SELECT id INTO crew_user_id FROM public.users WHERE auth_user_id = crew_auth_id;
        ...
        RAISE NOTICE 'Created crew user profile for auth_user_id: %', crew_auth_id;
    ELSE
...
BEGIN
    ...
    IF viewer_auth_id IS NOT NULL THEN
        ...
        DECLARE
            viewer_user_id UUID;
        BEGIN
            SELECT id INTO viewer_user_id FROM public.users WHERE auth_user_id = viewer_auth_id;
        ...
        RAISE NOTICE 'Created viewer user profile for auth_user_id: %', viewer_auth_id;
    ELSE
