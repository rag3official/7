...
-- =============================================================================
-- 1. ENSURE CORE TABLES EXIST WITH PROPER STRUCTURE
-- =============================================================================

-- Update driver_profiles table (your current structure is good, just ensure indexes)
CREATE INDEX IF NOT EXISTS idx_driver_profiles_slack_user_id ON public.driver_profiles(slack_user_id);
CREATE INDEX IF NOT EXISTS idx_driver_profiles_name ON public.driver_profiles(driver_name);

-- Create van_profiles table if it doesn't exist
...
-- Create indexes for van_profiles
...
-- =============================================================================
-- 2. UPDATE VAN_IMAGES TABLE TO PROPERLY LINK TO DRIVERS
-- =============================================================================

-- First, check if van_images table exists and add missing columns
DO $$
BEGIN
...
CREATE INDEX IF NOT EXISTS idx_driver_van_assignments_date ON public.driver_van_assignments(assigned_date DESC);

-- =============================================================================
-- 4. CREATE VIEWS FOR DRIVER PROFILE NAVIGATION
-- =============================================================================
...
SELECT 
    ...
    current_van.van_number as current_van_number,
    current_van.make as current_van_make,
    current_van.model as current_van_model
...
LEFT JOIN public.driver_van_assignments dva ON dp.id = dva.driver_id AND dva.is_current = true
LEFT JOIN public.van_profiles current_van ON dva.van_id = current_van.id;
...
BEGIN
    ...
    SELECT 
        ...
        vp.make as van_make,
        vp.model as van_model,
        CONCAT(vp.make, ' ', vp.model, ' (#', vp.van_number, ')') as van_display_name,
        ...
        jsonb_agg(
            jsonb_build_object(
                ...
                'image_data', CASE WHEN LENGTH(vi.image_data) > 100 THEN LEFT(vi.image_data, 100) || '...' ELSE vi.image_data END,
...
-- Function to get van's images with driver info
...
BEGIN
    ...
    SELECT 
        vi.id as image_id,
        vi.image_url,
        CASE WHEN LENGTH(vi.image_data) > 100 THEN LEFT(vi.image_data, 100) || '...' ELSE vi.image_data END as image_data,
        vi.van_damage as damage_description,
        vi.van_rating as damage_rating,
        vi.created_at as uploaded_at,
        dp.id as driver_id,
        dp.driver_name,
        COALESCE(dp.slack_real_name, dp.slack_display_name, dp.driver_name) as driver_display_name,
        dp.phone as driver_phone
    FROM public.van_images vi
    LEFT JOIN public.driver_profiles dp ON vi.driver_id = dp.id
...
GRANT EXECUTE ON FUNCTION public.get_van_images_with_drivers(int, int) TO authenticated, anon;
...
SELECT 'SUCCESS: Enhanced driver-van image linking system created!' as status,
       'Tables: driver_profiles, van_profiles, van_images, driver_van_assignments' as tables_created,
       'Views: driver_profile_summary, driver_images_with_van_details, van_images_with_driver_details' as views_created,
       'Functions: get_driver_images_by_van, get_van_images_with_drivers, link_images_to_drivers' as functions_created;
