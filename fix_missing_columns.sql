-- Fix Missing Columns in Vans Table
...
-- Check what columns exist in vans table
SELECT 'CURRENT VANS TABLE COLUMNS' as info, '' as details
UNION ALL
SELECT '==============================' as info, '' as details
UNION ALL
SELECT 
    column_name as info,
    CONCAT(data_type, CASE WHEN is_nullable = 'YES' THEN ' (nullable)' ELSE ' (not null)' END) as details
...
-- =============================================================================
-- 2. ADD MISSING COLUMNS TO VANS TABLE
-- =============================================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added last_maintenance_date column to vans table';
    ELSE
        RAISE NOTICE 'last_maintenance_date column already exists';
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added maintenance_notes column to vans table';
    ELSE
        RAISE NOTICE 'maintenance_notes column already exists';
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added created_at column to vans table';
    ELSE
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added updated_at column to vans table';
    ELSE
...
-- =============================================================================
-- 3. NOW RECREATE THE VIEWS (SAFELY)
-- =============================================================================
...
-- Recreate van_images_with_van view
...
-- Recreate van_images_with_driver view
...
-- Recreate active_driver_assignments view (simplified to avoid missing columns)
...
-- Check updated vans table structure
SELECT 'UPDATED VANS TABLE COLUMNS' as info, '' as details
UNION ALL
SELECT '==============================' as info, '' as details
UNION ALL
SELECT 
...
SELECT 'VIEW TESTS' as test_name, '' as result
UNION ALL
SELECT '===========' as test_name, '' as result
UNION ALL
SELECT 'van_images_with_van' as test_name, COUNT(*)::text as result FROM public.van_images_with_van
UNION ALL
SELECT 'van_images_with_driver' as test_name, COUNT(*)::text as result FROM public.van_images_with_driver
UNION ALL
SELECT 'active_driver_assignments' as test_name, COUNT(*)::text as result FROM public.active_driver_assignments;

-- Show sample data from vans table
SELECT 'SAMPLE VANS DATA' as info, '' as details
UNION ALL
SELECT '=================' as info, '' as details;

SELECT van_number, type, status, driver, created_at 
FROM vans 
LIMIT 3;

SELECT 'SUCCESS' as status, 'All columns added and views recreated' as message; 