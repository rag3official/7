...
def get_or_create_van(van_number):
    ...
    try:
        ...
        van_data = {
            ...
            'model': 'Unknown', 
            'year': 2020,
            'license_plate': f'VAN{van_number}',
            'status': 'active'
        }
...
def handle_file_shared(event, say):
    ...
    try:
        logger.info("=" * 50)
        logger.info("📁 PROCESSING FILE SHARED EVENT (RAW HTTP)")
        logger.info("=" * 50)
...
if __name__ == "__main__":
    logger.info("🚀 Starting RAW HTTP Slack Bot...")
    logger.info("📁 Focus: Raw HTTP uploads to bypass MIME type issues")
